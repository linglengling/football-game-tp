...
class AnWPFL_Referee extends CPT_Core {
	 ...
	 * @since 0.14.7
	 */
	public function get_game_referee_main( $game_id ) {

		static $games_map = null;

		if ( null === $games_map ) {
			$games_map = $this->get_game_referee_map();
		}

		return isset( $games_map[ $game_id ] ) ? $games_map[ $game_id ] : '';
	}

	/**
	 * Get map of game > main referee ID
	 *
	 * @return array $output_data
	 * @since 0.14.7
	 */
	public function get_game_referee_map() {

		$cache_key = 'FL-REFEREES-GAME-MAP';
		...
		$games_map    = [];
		$all_referees = $wpdb->get_results(
			"
			SELECT post_id, meta_value
			FROM $wpdb->postmeta
			WHERE meta_key = '_anwpfl_referee' AND meta_value > ''
			"
		);

		foreach ( $all_referees as $referee_obj ) {
			$games_map[ $referee_obj->post_id ] = $referee_obj->meta_value;
		}

		anwp_football_leagues()->cache->set( $cache_key, $games_map );
	 ...
	 */
	public function get_referee_games( $options, $result = '', $type = 'referee' ) {
		...
		if ( 'assistant' === $type ) {
			...
			$query .= "LEFT JOIN $wpdb->postmeta pm ON ( pm.post_id = g.match_id AND pm.meta_key = '_anwpfl_referee' )";
		}

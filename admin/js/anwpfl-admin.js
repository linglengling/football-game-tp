...
( function( window, document, $, plugin ) {
	...
	plugin.bindEvents = function() {
		...
		$c.body.on( 'click', '[data-fl-flush-plugin-cache]', function( e ) {
			...
			jQuery.ajax( {
				...
				url: anwp.rest_root + 'anwpfl/v1/helper/flush-plugin-cache'
			} ).always( function() {
	...
	plugin.recalculateStats = function() {

		const btn     = document.querySelector( '[data-anwpfl-recalculate-match-stats]' );

		if ( ! btn ) {
			return;
		}

		const stats   = document.querySelector( '[data-anwpfl-match-stats]' );
		const spinner = document.querySelector( '[data-anwpfl-recalculate-match-spinner]' );

		function recalculateSomeGames( reset = false ) {
			...
			jQuery.ajax( {
				...
				stats.textContent = `matches/stats - ${response.all}/${response.calc}`;

				if ( response.all > response.calc ) {
					recalculateSomeGames();
				} else {

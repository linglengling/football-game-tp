...
final class AnWP_Football_Leagues {
	...
	public function insert_thumbnail_id( $check, $object_id, $meta_key ) {
		...
		if ( ( ! is_search() && ! ( is_post_type_archive( [ 'anwp_player', 'anwp_referee', 'anwp_staff', 'anwp_stadium', 'anwp_club' ] ) ) ) || empty( $object_id ) ) {
			return $check;
		}
	...
	public function fix_filter_post_menu_title( $title, $post ) {
		...
		if ( '' === $post->post_title && ! empty( $post->object ) && in_array( $post->object, [ 'anwp_match', 'anwp_competition' ], true ) && 'no' !== anwp_football_leagues()->customizer->get_value( 'general', 'hide_post_titles' ) ) {
	...
	public function add_body_classes( $classes ) {
		...
		if ( 'no' !== anwp_football_leagues()->customizer->get_value( 'general', 'hide_post_titles' ) ) {
...
//		}
	...
	public function public_enqueue_scripts() {
		...
		*/
		...
		|--------------------------------------------------------------------------
		*/
		wp_enqueue_script( 'anwp-fl-public', self::url( 'public/js/anwp-fl-public.min.js' ), [], self::VERSION, true );
		...
		|--------------------------------------------------------------------------
		*/
		wp_register_script( 'anwp-fl-justified-gallery', self::url( 'vendor/flickr-justified-gallery/fjGallery.min.js' ), [], self::VERSION, true );
		wp_register_script( 'anwp-fl-justified-gallery-modal', self::url( 'vendor/baguette-box/baguetteBox.min.js' ), [], self::VERSION, true );
		...
		*/
		...
		|--------------------------------------------------------------------------
		*/
		$customizer_css = $this->customizer->get_customizer_css();
	...
	public function admin_enqueue_scripts() {
		...
		*/
		...
		if ( in_array( $current_screen->id, $plugin_pages, true ) ) {
			...
			*/
			if ( is_rtl() ) {
				wp_enqueue_style( 'anwpfl_styles_rtl', self::url( 'admin/css/styles-rtl.css' ), [ 'wp-color-picker' ], self::VERSION );
			} else {
				wp_enqueue_style( 'anwpfl_styles', self::url( 'admin/css/styles.css' ), [ 'wp-color-picker' ], self::VERSION );
			}
			...
			*/
			...
			|--------------------------------------------------------------------------
			*/
			wp_enqueue_script( 'anwpfl_admin_vue', self::url( 'admin/js/anwpfl-admin-vue.min.js' ), [ 'vuejs', 'anwpfl_admin', 'wp-color-picker' ], self::VERSION, true );
	...
	public static function is_premium_active() {
		return defined( 'ANWP_FL_PREMIUM_VERSION' );
	}
}
